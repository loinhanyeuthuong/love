...
$(document).on('submit', '#sendpwd', function (event) {
    ...
    $('.mailpreload').fadeIn(function(){
        $.ajax({
            type: "POST",
            url: "admin/ajax/sendpwd.php",
            data: serialize
            })
            .done(function( msg ) {
                $('.sendresponse').html(msg).fadeIn();
                $('.mailpreload').fadeOut();
                $('#captcha').attr('src', 'admin/captcha.php?' + time);
...
function createZip(folder, dash){
    ...
    $.ajax({
        cache: false,
        type: "POST",
        url: "admin/ajax/zip.php?t=" + (new Date).getTime(),
        data: {
            fold: folder,
            dash: dash
        }
        ...
        if (numfiles > 0) {            
            ...
            $('#sendfiles').unbind('submit').submit(function(event) {
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "admin/ajax/sendfiles.php?t=" + now,
            ...
            if (numfiles >= $maxzipfiles) {
                ...
                .done(function( msg ) {
                    if ($prettylinks) {
                        ...
                        shortlink = 'admin/vfm-downloader.php?dl=' + msg;
                    }
...
function setupMove($activepagination, $selectfiles, $time, $hash, $doit) {
    ...
    $(document).on('click', '.movelink', function(e) {
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: "admin/vfm-move.php?t=" + (new Date).getTime(),
            data: movedata
        })
        .done(function(msg) {
            if (msg == 'ok') {
