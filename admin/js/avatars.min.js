...
/*! VFM avatar setup */
function Avatars(a,e){$(".uppa").on("click",function(){$(".updated").html(""),$("#uppavatar").trigger("click")}),$("#uppavatar").on("change",function(){$(".export").removeClass("hidden")}),$(".slider").on("change",function(){$(".updated").html("")}),$(".image-editor").cropit({imageState:{src:a}}),$(".remove-avatar").on("click",function(){$(".updated").html(""),$(".image-editor").cropit("imageSrc",e),$(".export").removeClass("hidden")}),$(".export").click(function(){var a=$('<i class="fa fa-check-circle fa-lg pull-left text-success"></i>'),e=$(".image-editor").cropit("export"),t=$(".image-name").val();$.ajax({method:"POST",url:"admin/ajax/avatar.php",data:{imgData:e,imgName:t}}).done(function(e){d=new Date,$(".updated").html(a),$(".avatar").attr("src",e+"?"+d.getTime()),$(".export").addClass("hidden")}).fail(function(){console.log("new avatar failed")})})}